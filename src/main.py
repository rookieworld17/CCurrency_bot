...
def send_welcome(message):
    bot.send_message(message.chat.id, get_welcome_message(), parse_mode="MarkdownV2")
...
def handle_message(message):
    ...
    if intent == 'forward':
        try:
            bot.reply_to(message, convert_currency(amount, symbol, intent=intent), parse_mode='MarkdownV2')
        except Exception:
    ...
    if intent == 'info':
        if not symbol.isalpha() or len(symbol) > 10:
            ...
            return
        try:
            bot.reply_to(message, get_token_info(symbol), parse_mode='MarkdownV2')
        except Exception:
